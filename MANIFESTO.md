...
To prevent others from making the same mistakes, we are transparent about all kind of incidents. Talking about failure or incidents must therefore always be open and blameless. To gain full transparency we recommend implementing the following points: Report about the incidents openly with reporting what happened, how you solved the incident and how you want to improve, make it comprehensible with setting a link to a unique identifier that covers the incident at the status page of the project; be also transparent about the history of this incident, but do not name people directly, use, if needed, reductions instead. But keep in mind that publishing incidents is only in the scope of public services. Incident reports that only occurred on a specific customer setup should be sent out to the customer and can be published with the customer´s agreement. When publishing incidents we encourage using well-established guidelines (e.g. Google SRE, Atlassian Blameless Postmortem, etc.), best sign organizations to act according to the SRE guideline. This manifest and all our work is based on the SRE movement – kudos!
